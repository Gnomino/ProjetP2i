...
public class MusicDatabase {
    ...
    public static LinkedList<Position> getNotesForMusic(Music music) throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM Position WHERE Position" +
                ".nomNote = Position.idMusique = ? ORDER BY Position.ordre ASC");
        preparedStatement.setInt(1, music.getIdMusic());

        ResultSet rs = preparedStatement.executeQuery();
        LinkedList<Position> res = new LinkedList<>();
        while (rs.next()) {
            res.add(new Position(music, notes.get(rs.getString("Note.nomNote")), rs.getInt("date")));
        }
        return res;
    }
    ...
    public static LinkedList<Attempt> getAllAttempts() throws SQLException {
        ...
        ResultSet rs =
                connection.createStatement().executeQuery("SELECT * FROM Tentative, Musique, NoteJouee " +
